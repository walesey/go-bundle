...
func TestParserErr(t *testing.T) {
    ...
    `, "(anonymous): Line 3:9 Illegal break statement")
	{
		...
		assert.Equal(t, parser.position(stmt.From).Column, 9)
		assert.Equal(t, parser.position(stmt.To).Column, 16)
		assert.Equal(t, parser.slice(stmt.From, stmt.To), "break; ")
	}
...
func TestParser(t *testing.T) {
	...
	assert.Equal(t, program.Body[0].(*ast.EmptyStatement).Semicolon, file.Idx(1))

	program = test(";;", nil)
	assert.Equal(t, len(program.Body), 2)
	assert.Equal(t, program.Body[0].(*ast.EmptyStatement).Semicolon, file.Idx(1))
	assert.Equal(t, program.Body[1].(*ast.EmptyStatement).Semicolon, file.Idx(2))

	program = test("1.2", nil)
	assert.Equal(t, len(program.Body), 1)
	assert.Equal(t, program.Body[0].(*ast.ExpressionStatement).Expression.(*ast.NumberLiteral).Literal, "1.2")
	...
	program = test("\n", nil)
	assert.Equal(t, len(program.Body), 0)
...
func TestPosition(t *testing.T) {
	...
	assert.Equal(t, parser.slice(0, 1), "")
	assert.Equal(t, parser.slice(0, 10), "")

	// Out of range, idx1 (error condition)
	assert.Equal(t, parser.slice(1, 128), "")

	assert.Equal(t, parser.str[0:0], "")
	assert.Equal(t, parser.slice(1, 1), "")

	assert.Equal(t, parser.str[0:1], "/")
	assert.Equal(t, parser.slice(1, 2), "/")

	assert.Equal(t, parser.str[0:14], "// Lorem ipsum")
	assert.Equal(t, parser.slice(1, 15), "// Lorem ipsum")
	...
	assert.Equal(t, node.Idx0(), file.Idx(2))
	assert.Equal(t, node.Idx1(), file.Idx(25))
	assert.Equal(t, parser.slice(node.Idx0(), node.Idx1()), "function(){ return 0; }")
	assert.Equal(t, parser.slice(node.Idx0(), node.Idx1()+1), "function(){ return 0; })")
	assert.Equal(t, parser.slice(node.Idx0(), node.Idx1()+2), "")
	assert.Equal(t, node.(*ast.FunctionLiteral).Source, "function(){ return 0; }")
	...
	assert.Equal(t, err, nil)
	node = program.Body[0].(*ast.ExpressionStatement).Expression.(*ast.FunctionLiteral)
	assert.Equal(t, node.(*ast.FunctionLiteral).Source, "function(){ return abc; }")
}
