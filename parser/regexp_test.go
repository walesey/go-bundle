...
func TestRegExp(t *testing.T) {
...

	{
		...
		test := func(input, expect string, expectErr interface{}) {
			...
			if err != nil {
				assert.Equal(t, err.Error(), expectErr)
			}
		}
...

	{
		...
		test := func(input string, expect string) {
			...
			if assert.Equal(t, result, expect) {
...
func TestTransformRegExp(t *testing.T) {
	...
	assert.Equal(t, pattern, `\s+abc\s+`)
	assert.Equal(t, regexp.MustCompile(pattern).MatchString("\t abc def"), true)
}
